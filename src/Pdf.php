...
namespace Fimacom\Pdf;

use Fimacom\Pdf\Common\Fpdf;

class Pdf extends Fpdf
{
    /**
     * tabela de codigos 128
     * @var array
     */
    private array $t128;
    private string $abcSet = "";                                        // conjunto de caracteres legiveis em 128
    private string $aSet = "";                                          // grupo A do conjunto de de caracteres legiveis
    private string $bSet = "";                                          // grupo B do conjunto de caracteres legiveis
    private string $cSet = "";                                          // grupo C do conjunto de caracteres legiveis
    private array $setFrom = ["A" => 0, "B" => 0, "C" => 0];         // converter de
    private array $setTo = ["A" => 0, "B" => 0, "C" => 0];           // converter para
    private array $jStart = ["A" => 103, "B" => 104, "C" => 105];      // Caracteres de seleção do grupo 128
    private array $jSwap = ["A" => 101, "B" => 100, "C" => 99];      // Caracteres de troca de grupo
     ...
     * @return void
     */
    public function code128(float $x, float $y, string $code, float $w, float $h)
    {
        $Aguid = "";
        $Bguid = "";
        $Cguid = "";
        ...
        $check = ord($crypt[0]);
        for ($i = 0, $iMax = strlen($crypt); $i < $iMax; $i++) {
            $check += (ord($crypt[$i]) * $i);
        }
        $check %= 103;
     ...
     * @return void
     */
    public function roundedRect(float $x, float $y, float $w, float $h, float $r, string $corners = '1234', string $style = '')
    {
        $k = $this->k;
        $hp = $this->h;
        if ($style === 'F') {
     ...
     * @return void
     */
    private function arc(float $x1, float $y1, float $x2, float $y2, float $x3, float $y3)
    {
        $h = $this->h;
        $this->out(
            sprintf(
     ...
     * @return void
     */
    public function dashedRect(float $x1, float $y1, float $x2, float $y2, float $width = 1, float $nb = 15)
    {
     ...
     * @return void
     */
    public function drawTextBox(string $strText, float $w, float $h, string $align = 'L', string $valign = 'T', bool $border = true)
    {
        $xi = $this->getX();
        $yi = $this->getY();
        $hrow = $this->fontSize;
     ...
     * @return int
     */
    public function wordWrap(string &$text, float $maxwidth): int
    {
        $text = trim($text);
        if ($text === '') {
            return 0;
     ...
     * @param string $link
     */
    public function cellFitScale(float $w, float $h = 0, string $txt = '', $border = 0, $ln = 0, string $align = '', $fill = false, string $link = '')
    {
     ...
     * @return  int
     */
    private function mbGetStringLength(string $s)
    {
     ...
     * @param float $x Posição horizontal inicial, em mm
     * @param float $y Posição vertical inicial, em mm
     * @param float $w Comprimento da linha, em mm
     * @param float $h Espessura da linha, em mm
     * @param float $n Numero de traços na seção da linha com o comprimento $w
     * @return void
     */
    public function dashedHLine(float $x, float $y, float $w, float $h, float $n)
    {
     ...
     * @param float $x Posição horizontal inicial, em mm
     * @param float $y Posição vertical inicial, em mm
     * @param float $w Espessura da linha, em mm
     * @param float $yfinal posição final
     * @param float $n Numero de traços na seção da linha com o comprimento $w
     * @return void
     */
    public function dashedVLine(float $x, float $y, float $w, float $yfinal, float $n)
    {
     ...
     * @param float $width
     * @param array $aFont
     * @return float numero de linhas
     */
    public function getNumLines(
        string $text,
        float $width,
        array $aFont = ['font' => 'Times', 'size' => 8, 'style' => '']
    ): float {
        $text = trim($text);
